...
module.exports = client => ({
  ...
  async execute(message) {
    ...
    const guildsData = (message.guild) ? (await guilds.get(message.guild.id)) : { prefix, language: language.substr(0, 2), blacklisted: false };
    ...
    if (message.content.replace(/\!/g, '').startsWith(`<@${client.user.id}>`)) {
      ...
      .setDescription(`> **Public Prefix: ${prefix}**\n> **Private Prefix: ${client.prefix}**`)
    ...
    if (message.guild && ["balance", "transfer", "setserver"].includes(command.name.toLowerCase())) return;
    if (!command.owners) {
      ...
      if (timestamps.has(message.author.id)) {
        ...
        if (now < expirationTime) {
          ...
          if (!cooldowns.has(message.author.id)) {
            ...
            return message.lineReplyNoMention(client.replys.public.cooldowns.replace(/\{timeLeft}/g, `${timeLeft.toFixed(1)}`))
              .then(msg => {    
    ...
    if (command.admin && !message.member.hasPermission(command.permissions)) return;
